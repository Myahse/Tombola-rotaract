import { clubDisplayName, escapeHtml, firstName, siteUrl, wrapEmail } from "./layout.js";
...
export function welcomeEmail(data: WelcomeEmail) {
  ...
  const html = wrapEmail({
    ...
    ctaUrl: buyUrl,
    bodyHtml: `
      <p style="margin:0 0 14px;color:#141416;">Vous faites maintenant partie de la tombola de <strong>${escapeHtml(clubDisplayName())}</strong>. Un compte, toutes les tombolas : cérémonie, réunion, ou la prochaine surprise du club.</p>
  ...
  const text = [
    ...
    "",
    "Votre compte est prêt. Un compte pour toutes les tombolas.",

...
export function ReceiptUploadSheet({
  ...
  useEffect(() => {
    ...
    if (file.type.startsWith("image/") || file.name.toLowerCase().endsWith(".jpg") || file.name.toLowerCase().endsWith(".png")) {
    ...
    <ModalPortal>
      ...
      >
        ...
        >
          ...
          <h1 id="receipt-upload-title">{title}</h1>
          <p className="modal-lead">{help}</p>
          {uploaded ? <p className="field-ok">{t("receiptUpload.sent")}</p> : null}
